...
- [x] **RF1: Remove `unwrap()` calls from `Agent::complete()` in production code**
  - In `synapse-core/src/agent.rs` lines 106-107, replace `messages.last().unwrap()` and `.tool_calls.clone().unwrap()` with safe alternatives (e.g., extract tool_calls before pushing the response, or use `if let` / `.ok_or()`).
  - **Acceptance:** No `unwrap()` or `expect()` calls exist in non-test code within `synapse-core/src/agent.rs`. `cargo test` passes.

- [x] **RF2: Fix `build_api_messages()` to include tool_calls on assistant messages for Anthropic provider**
  - In `synapse-core/src/provider/anthropic.rs`, update `build_api_messages()` to detect when an `Assistant` message has `tool_calls` populated and serialize them as `tool_use` content blocks in the Anthropic API format (array of `ContentBlock` with `type: "tool_use"`, `id`, `name`, `input`).
  - Add test `test_assistant_tool_call_message_serialization` that verifies an assistant message with `tool_calls` is serialized with `tool_use` content blocks.
  - **Acceptance:** Assistant messages with `tool_calls` include `tool_use` blocks when serialized for the Anthropic API. New test passes. Existing tests pass.

- [x] **RF3: Fix `build_api_messages()` to include tool_calls on assistant messages for OpenAI provider**
  - In `synapse-core/src/provider/openai.rs`, update `build_api_messages()` to check if the source `Message` has `tool_calls` populated and, if so, convert them to `Vec<OpenAiToolCall>` on the `ApiMessage`.
  - Add test `test_assistant_tool_call_message_serialization` that verifies an assistant message with `tool_calls` is serialized with the `tool_calls` field containing the correct `OpenAiToolCall` entries.
  - **Acceptance:** Assistant messages with `tool_calls` include the `tool_calls` field in the serialized API message. New test passes. Existing tests pass.

- [x] **RF4: Fix `build_api_messages()` to include tool_calls on assistant messages for DeepSeek provider**
  - In `synapse-core/src/provider/deepseek.rs`, update `build_api_messages()` to check if the source `Message` has `tool_calls` populated and, if so, convert them to `Vec<DeepSeekToolCall>` on the `ApiMessage`.
  - Add test `test_assistant_tool_call_message_serialization` that verifies an assistant message with `tool_calls` is serialized with the `tool_calls` field containing the correct `DeepSeekToolCall` entries.
  - **Acceptance:** Assistant messages with `tool_calls` include the `tool_calls` field in the serialized API message. New test passes. Existing tests pass.
