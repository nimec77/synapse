...
- [x] **Task 1: Add `ratatui` and `crossterm` dependencies**
...
- [x] **Task 2: Add `--repl` flag to CLI args**
...
- [x] **Task 3: Implement TerminalGuard for cleanup safety**
...
- [x] **Task 4: Implement ReplApp state and UI rendering**
...
- [x] **Task 5: Implement key handling and input editing**
...
- [x] **Task 6: Implement async event loop with streaming**
...
- [x] **Task 7: Implement session persistence in REPL**
...
- [x] **Task 8: Implement session resume (`--repl --session <id>`)**
...
- [x] **RF1: Fix duplicate conversation history when resuming a session**
  - In `run_repl()` (repl.rs lines 540-550), the conversation message vector is built by first collecting all messages from `history` (the stored messages loaded at session resume), then appending ALL of `app.messages`. However, `app.messages` already contains the history messages (populated at lines 471-476 during session resume). This means when resuming a session, every historical message is sent to the LLM twice.
  - Fix: When building `conv_messages`, either (a) only use `app.messages` as the source of truth (skip the `history` iteration entirely), or (b) only append messages from `app.messages` that were added during the current REPL session (i.e., those not from the initial history population). Option (a) is simpler and more correct since `app.messages` already contains the full conversation.
  - **Acceptance:** When resuming a session with `--repl --session <id>`, the LLM receives each historical message exactly once, followed by the new user message. A unit test should verify no duplication occurs.
